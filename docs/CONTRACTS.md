...
`sast.todo-security` (info: `TODO`/`FIXME` mentioning auth/security),
`sast.assistant-form-no-working-state` (low: a form the agent wrote posts to a route that asks the assistant and has no `data-working`; see SC-26). Findings carry `introducedBy` from provenance.
...
SC-26 a form whose action takes noticeable time carries `data-working="<what is happening>"` and states the expected
wait in plain text beside the button — the shared script disables the button and announces the message, and the
sentence is what a person without JavaScript has to go on. Checked in part: `sast.assistant-form-no-working-state` finds
a form the agent wrote that posts to a route asking the assistant (through the assistant's own client or providers) and
has no `data-working`. Not checked: any other slow route, since nothing decides for us which routes are slow; a page
that reaches a model some other way; and the sentence about the wait, which a pattern cannot judge. A generated page
that omits those produces no finding.
