...
sentence is what a person without JavaScript has to go on. Told to the agent, not enforced by a scan: nothing
decides for us which routes are slow, so a generated page that omits it produces no finding.
