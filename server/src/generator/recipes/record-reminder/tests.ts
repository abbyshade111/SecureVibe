...
export function reminderTests(plan: ReminderPlan): EmittedTest[] {
  ...
  tests.push({
    ...
    code: `  test('${name}: a reminder goes out once for a ${lower} that is coming up, and not for one that is far off', async (t) => {
    ...
    assert.equal(mail.filter((m) => m.includes(later)).length, 0, 'nothing may be sent about a ${lower} a week away');
    ...
    assert.equal(remindedAt(soon) === null, false, 'the ${lower} must be marked as reminded');
    assert.equal(remindedAt(later), null, 'the ${lower} a week away must not be marked');
  });`,
  });
  ...
  tests.push({
    ...
    assert.deepEqual(repeated, [], 'no ${lower} may be reminded about twice');
  });`,
  });
...
/** Reminders about ${(entity.entity.pluralLabel ?? `${entity.entity.label}s`).toLowerCase()}: who is told, how often, and what the message carries. */
...
describe('${entity.entity.name} reminders', () => {
  ...
  /** A value for the ${plan.field.label.toLowerCase()} field, \`hours\` from now. */
