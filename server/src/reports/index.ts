...
import { htmlToPdf, type PageSize } from './pdf/index.js';
 ...
 */
async function writePdfArtifact(outDir: string, htmlName: string, html: string, kind: ArtifactRef['kind'], description: string, createdAt: string, pageSize?: PageSize): Promise<ArtifactRef[]> {
  try {
    const name = htmlName.replace(/\.html$/, '.pdf');
    const path = join(outDir, name);
    await writeFile(path, htmlToPdf(html, { createdAt, ...(pageSize ? { pageSize } : {}) }));
    const s = await stat(path);
...
export async function renderReportsFromModel(model: ReportModel): Promise<RenderReportsResult> {
  ...
  artifacts.push(...(await writePdfArtifact(model.outDir, 'overview.html', overview, 'overview', 'The one-page summary as a PDF file.', madeAt, model.pdfPageSize)));
  ...
  artifacts.push(...(await writePdfArtifact(model.outDir, 'compliance-report.html', compliance.html, 'compliance-report', 'Full compliance report as a PDF file.', madeAt, model.pdfPageSize)));
  ...
  artifacts.push(...(await writePdfArtifact(model.outDir, 'security-report.html', security.html, 'security-report', 'Security report as a PDF file.', madeAt, model.pdfPageSize)));
  ...
  if (needsGoingOnline(model)) {
    ...
    artifacts.push(...(await writePdfArtifact(model.outDir, 'going-online.html', goingOnline.html, 'going-online-checklist', 'Going-online checklist as a PDF file.', madeAt, model.pdfPageSize)));
  }
...
export async function renderReportsWithoutAnswers(input: Omit<RenderReportsInput, 'design' | 'compliance'>): Promise<RenderReportsResult> {
  ...
  const model: SecurityReportInput = {
    ...
    ...(input.pdfPageSize ? { pdfPageSize: input.pdfPageSize } : {}),
  };
  const artifacts: ArtifactRef[] = [];
  const madeAt = model.run.finishedAt ?? model.run.startedAt;
  ...
  artifacts.push(...(await writePdfArtifact(model.outDir, 'overview.html', overview, 'overview', 'The summary as a PDF file.', madeAt, model.pdfPageSize)));
  const security = renderSecurityReport(model);
  artifacts.push(await writeFileArtifact(model.outDir, 'security-report.html', security.html, 'security-report', 'html', 'Every finding with what it is, why it matters and how to fix it.'));
  artifacts.push(await writeFileArtifact(model.outDir, 'security-report.md', security.md, 'security-report', 'md', 'Security report in Markdown.'));
  artifacts.push(...(await writePdfArtifact(model.outDir, 'security-report.html', security.html, 'security-report', 'Security report as a PDF file.', madeAt, model.pdfPageSize)));
