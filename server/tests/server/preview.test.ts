...
function fetchPage(port: number, path: string): Promise<{ status: number; body: string }> {
  return new Promise((resolve) => {
    const req = http.request({ host: '127.0.0.1', port, path, method: 'GET', timeout: 3_000 }, (res) => {
      let body = '';
...
describe('app preview', () => {
  ...
  it('runs the built app with a one-time sign-in and throws its data away when stopped', async () => {
    ...
    // The link signs the visitor in: no password step for someone trying the app out. It answers with a page of
    // the app's own rather than a redirect, and that is the point rather than a detail: the visitor arrives from
    // SecureVibe on 127.0.0.1 while the preview is on localhost, which a browser counts as another site, and it
    // withholds the SameSite=Strict session cookie from anything that cross-site click goes on to request. A
    // redirect therefore landed on the home page with no cookie and asked for a password. Nothing here can see
    // that — a test has no notion of which site started a request — so this assertion stands in for it.
    const handover = await fetchPage(port, new URL(info.url!).pathname + new URL(info.url!).search);
    expect(handover.status).toBe(200);
    expect(handover.body).toContain('http-equiv="refresh"');
