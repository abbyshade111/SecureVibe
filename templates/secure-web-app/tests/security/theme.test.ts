 ...
 *
 * It also checks that the app only ever renders a theme it knows: the name goes into an HTML attribute, so an
 * unknown value is refused at startup rather than written into the page.
 */
...
import { expectStartupRefusal, startApp, templateRoot, type RunningApp } from '../helpers/app.ts';
...
describe('themes', () => {
  test('UX-01 the stylesheet defines every theme the app offers, light and dark', () => {
  ...
  test('UX-01 every theme stays readable: WCAG AA contrast in both the light and the dark version', () => {
  ...
  test('UX-01 the stylesheet names no colour outside a theme, so a theme really does change everything', () => {
  ...
  describe('the rendered page', () => {
    ...
    test('UX-01 the page carries the chosen theme, and the default when nothing was chosen', async () => {
